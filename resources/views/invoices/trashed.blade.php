...
@section('content')
        ...
        <div class="col-xl-12">
            <div class="card mg-b-20">
                <div class="card-header pb-0">
                    <a href="{{ route('invoices.index') }}" class="modal-effect btn  btn-primary" style="color:white">
                        <strong>رجوع</strong>
                    </a>
                                        ...
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu tx-13">
                                                    ...
                                                    @can('استعادة فاتورة')
                                                    <a class="dropdown-item href="#" data-invoice_id="{{ $invoice->id }}"
                                                        data-toggle="modal" data-target="#re_invoice"><i
                                                            class="text-danger typcn typcn-arrow-back-outline"></i>&nbsp;&nbsp;
                                                        استعادة </a>
                                                        @endcan
                                                </div>
                                            </div>
    ...
    {{-- استعادة من الارشيف --}}
    <div class="modal fade" id="re_invoice" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">استعادة من الارشيف </h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                    <form action="{{ route('retrached') }}" method="post">
                        {{ csrf_field() }}
                        @method('PUT')
                </div>
                <div class="modal-body">
...
@section('js')
    ...
    <script>
        ...
        $('#re_invoice').on('show.bs.modal', function(event) {
            var button = $(event.relatedTarget)
            var invoice_id = button.data('invoice_id')
            var modal = $(this)
            modal.find('.modal-body #invoice_id').val(invoice_id);
